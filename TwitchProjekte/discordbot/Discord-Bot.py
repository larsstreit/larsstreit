...
TOKEN = 'ODMxMjAxNTk5NDI1NjA5NzU5.YHRy0A.Fa269JWbqq_Tg0WvNlEYAbOfjFw' # Bot Token goes here ! #
...
guild_ids = [824759663383412786] # Your Guild ID goes here (multiple guilds possible) #
client.remove_command('help')
status = cycle(['Twitch.TV', 'Music']) # Standard Games can be edited if needed #
ROLE = 'Paradiesvogel' # Standard Role can be edited when needed ! #
...
async def on_ready():
    ...
    BCon = BCon = 'Online'
    Build = Build ='6.2.1'
    Copyright = Copyright ='© 2022 Shinyhunter.TV'
    change_status.start()
...
# (optional) This Module can be filled out to get the reddit feature working ! | Currently Alpha !
...
async def SecurityVer(ctx):
    SecurityVer = SecurityVer = 2.9
    NewSecVer = NewSecVer = 2.9
...
async def Server_Status(ctx):
    ...
    await ctx.send(f'The Status of the Server is currently **{Server_Status}**')
...
async def report(ctx, member: discord.Member,  *, arg):
    role = ctx.guild.get_role(824760224770162759) # enter role here #
...
async def PV(ctx):
    guild = ctx.guild
    await guild.create_role(name="Paradiesvogel")
    await asyncio.sleep(5)
    await ctx.send(f'**Role has been created**')
...
async def vip_dm(ctx):
    guild = client.get_guild(id=824759663383412786) # guild ID here #
    role = discord.utils.get(guild.roles, id=842190181758926899) # vip role goes here #
...
async def keyword(ctx, *, word: str):
    channel = client.get_channel(842188700318302258) # selected channel for keyword search #
...
async def PreRelease(ctx):
    ...
    embed.add_field(name='Pre-Release Version' ,value='[Click here to download]( https://github.com/Shinyhunter2109/Discord-Moveset-Bot/releases/download/6.2.1/Discord-Moveset-Bot.7z )', inline=False)
    await ctx.send(embed=embed)
...
@slash.slash(name="chnick", guild_ids=guild_ids)
@commands.has_permissions(administrator=True)
...
async def on_member_update(before, after):
        if after.guild.id == 824759663383412786: # Your Guild ID here #
            if before.activity == after.activity:
                return

            role = get(after.guild.roles, id=936546300621627402) # Your Stream Role goes here #
            channel = get(after.guild.channels, id=824759665674027090) # Stream channel goes here #
