...
ROLE = 'Rolle hier einfügen...'


@client.event
async def on_ready():
    change_status.start()
    print('Du bist eingeloggt als: ' + client.user.name + '\n')
    print('Diser Discord Bot wurde gemacht von twitch.tv/shinyhunter2109')
    print('Bot Version: 3.5')
    print('Du bist auf der neuesten Version')
...
async def on_error(event, *args, kwargs):
    ...
    await client.send_message(message.channel, "Fehler aufgetreten!")
...
async def delta(ctx, *, member: JoinDistance):
    ...
    if is_new:
        await ctx.send("Hey du bist noch sehr neu hier.")
    else:
        await ctx.send("Du bist schon länger hier.")
...
async def roles(ctx, *, member: MemberRoles):
    """Sagt welche Rollen ein Nutzer hat"""
    await ctx.send('Ich sehe die folgenden Rollen: ' + ', '.join(member))


@client.event
async def on_command_error(ctx, error):
    if isinstance(error, commands.CommandNotFound):
        await ctx.send('**Falscher Command verwendet**')


@client.event
async def on_permission_error(ctx, error):
    if isinstance(error, commands.MissingPermissions):
        await ctx.send('**Du hast nicht die Berechtigungen diesen Befehl zu verwenden!**')


@client.command()
async def joined(ctx, *, member: discord.Member):
    await ctx.send('{0} beigetreten am {0.joined_at}'.format(member))


class Slapper(commands.Converter):
    async def convert(self, ctx, argument):
        to_slap = random.choice(ctx.guild.members)
        return '{0.author} schlug {1} wegen *{2}*'.format(ctx, to_slap, argument)
...
def is_it_me(ctx):
    return ctx.author.id == 'Discord ID hier einfügen'
...
async def help(ctx):
    ...
    embed.add_field(name='.ping', value='gibt Pong! wieder', inline=False)
...
async def coinhelp(ctx):
    ...
    embed.add_field(name='.coinflip', value='gibt entweder Heads/Tails zurück', inline=False)
    await ctx.send(author, embed=embed)
...
async def joinhelp(ctx):
    ...
    embed.add_field(name='.join', value='Gibt wieder ob der Beitritt von dem Bot erfolgreich war', inline=False)
    await ctx.send(author, embed=embed)
...
async def pokehelp(ctx):
    ...
    embed.add_field(name='.pokemonname', value='gibt Info & GIF Bild wieder', inline=False)
    await ctx.send(author, embed=embed)
...
async def bottlehelp(ctx):
    ...
    embed.add_field(name='.bottles', value='Gibt die [Anzahl] des Biers wieder', inline=False)
    await ctx.send(author, embed=embed)
...
async def spotifyhelp(ctx):
    ...
    embed.add_field(name='.spotify', value='Gibt die Spotify Aktivität wieder', inline=False)
    await ctx.send(author, embed=embed)
...
async def eightballhelp(ctx):
    ...
    embed.add_field(name='.8ball', value='Gibt eine Antwort', inline=False)
    await ctx.send(author, embed=embed)
...
async def musichelp(ctx):
    ...
    embed.add_field(name='.play', value='Gibt Musik wieder', inline=False)
    await ctx.send(author, embed=embed)
...
async def kickhelp(ctx):
    ...
    embed.add_field(name='.kick', value='Kickt Leute vom Server', inline=False)
    await ctx.send(author, embed=embed)
...
async def banhelp(ctx):
    ...
    embed.add_field(name='.ban', value='Bannt Leute vom Server', inline=False)
    await ctx.send(author, embed=embed)
...
async def blackjackhelp(ctx):
    ...
    embed.add_field(name='.blackjack', value='Gibt entweder Sieg Niederlage oder Unentschieden wieder', inline=False)
    await ctx.send(author, embed=embed)
...
async def spotify(ctx, user: discord.Member=None):
    ...
    for activity in user.activities:
        if isinstance(activity, Spotify):
            await ctx.send(f'{user} hört gerade {activity.title} von {activity.artist}') # Tells you to what someone is listening


@spotify.error
async def spotify_error(ctx, error):
    if isinstance(error, commands.CommandOnCooldown):
        msg = '**Dieser Command hat Cooldown {:.2f}s**'.format(error.retry_after)
        await ctx.send(msg)
    else:
        raise error
...
async def emoji(ctx, emoji: discord.PartialEmoji = None):
    if not emoji:
        await ctx.send('**Einen Emote angeben**')
    else:
...
async def backup(ctx):
    await ctx.send('**Noch in der Entwicklung! | Fehler: 0830-5219-8607 |**')
    await ctx.send('**https://open.spotify.com/track/6R6on0ldJcSDfi2whJziJ1?si=NcphntXxRN67ht26nMYIjQ**')


@client.command()
async def sendembed(ctx):
    e = discord.Embed(title="Stack Overflow -",
...
async def join(ctx):
    ...
    if voice and voice.is_connected():
        ...
        print(f'Bot ist beigetreten {channel}')
        await ctx.send(f'Betrete {channel}')
    else:
        print('Bot befindet sich im Kanal')
...
async def leave(ctx):
    ...
    if voice and voice.is_connected():
        ...
        await ctx.send(f'Verlasse {channel}')
...
async def play(ctx, url: str):
    ...
    try:
        ...
        await ctx.send("Warte darauf das die Playlist endet!")
...
async def Update(ctx):
    ...
    await ctx.send(f'https://github.com/Shinyhunter2109/Discord-Moveset-Bot/releases/download/3.5/Discord-Moveset-Bot.7z')
    await asyncio.sleep(20)
